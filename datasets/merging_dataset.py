...
class Merging_data_set() : 
    ...
    def transform( self, HR , LR , mask ):
        # Resize
        resize = A.Resize( height= self.img_height ,  width = self.img_width , always_apply=True )
    ...
    def __getitem__(self,item) : 
        ...
        HR  = sitk.GetArrayFromImage(self.image[int(self.sub_id[item])][:,:,int(self.slice[item])])/100
        LR  = sitk.GetArrayFromImage(self.image[int(self.sub_id[item])][:,:,int(self.slice[item])])/100
